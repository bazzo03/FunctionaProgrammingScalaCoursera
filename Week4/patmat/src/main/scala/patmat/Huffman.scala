...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    else pack(chars) map (x => (x.head, x.length))
  }

  private def pack[Char](xs: List[Char]): List[List[Char]] = xs match {
    case Nil => Nil
    case x :: xs1 =>
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    @annotation.tailrec
    def createLeafList(frequencies: List[(Char, Int)], leafList : List[Leaf]) : List[Leaf] = {

      frequencies match {
        case Nil => leafList
        case x :: xs =>
          val list = leafList ::: List(createLeaf(x._1, x._2))
          createLeafList(frequencies.tail, list)
      }
    }
    createLeafList(freqs, List()).sortWith((x:Leaf, y:Leaf) => x.weight < y.weight)
  }

  private def createLeaf(char: Char, weight: Int): Leaf = {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case x :: y :: xs => (makeCodeTree(x, y) :: xs).sortWith((codeTree1 : CodeTree, codeTree2 : CodeTree) => weight(codeTree1) < weight(codeTree2))
      case _ => trees
    }
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    val list: List[(Char, Bit)] = times(chars)
    val leafList: List[Leaf] = makeOrderedLeafList(list)
    val tree: List[CodeTree] = until(singleton, combine)(leafList)
    tree.head
  }
